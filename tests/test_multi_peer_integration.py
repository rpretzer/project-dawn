...
async def test_peer_registry_persistence():
    ...
    try:
        # Create first node
        node1 = P2PNode(identity=NodeIdentity(), address="ws://localhost:8000")
        node1.peer_registry.data_dir = temp_dir / "mesh"
        node1.peer_registry.data_dir.mkdir(parents=True, exist_ok=True)
        node1.peer_registry.peer_registry_path = node1.peer_registry.data_dir / "peer_registry.json"
        node1.peer_registry.clear()
        ...
        node2.peer_registry.peer_registry_path = node2.peer_registry.data_dir / "peer_registry.json"
...
async def test_multiple_peers_in_registry():
    ...
    temp_dir = Path(tempfile.mkdtemp())
    try:
        node = P2PNode(identity=NodeIdentity(), address="ws://localhost:8000")
        node.peer_registry.data_dir = temp_dir / "mesh"
        node.peer_registry.data_dir.mkdir(parents=True, exist_ok=True)
        node.peer_registry.peer_registry_path = node.peer_registry.data_dir / "peer_registry.json"
...
async def test_peer_health_tracking():
    ...
    node = P2PNode(identity=NodeIdentity(), address="ws://localhost:8000")
    node.peer_registry.persist = False  # Disable persistence for this test
    node.peer_registry.clear()
